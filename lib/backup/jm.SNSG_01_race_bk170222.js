...
//console.log(nNum,hD[nNum]['rrD'][0],hD[nNum]['rrD'][1],hD[nNum]['rrD'][2],hD[nNum]['rrD'][3]);
			}
...
/*
		setInfoWaku : function ( tID, lap, nn ) {
			var wC = this.def.waku[lap-1];
			$('table#'+tID+' > tbody > tr.'+nn).append( $("<td>").append('　').css('background-color','rgb('+wC[0]+','+wC[1]+','+wC[2]+')') );
		},//END ();
		resetInfoTD : function ( tID, lap, nn, nc ) {
			$('table#'+tID+' > tbody > tr.'+nn+' > td.'+nc).html( lap );
		},//END ();
		setInfoTD : function ( tID, lap, nn, nc ) {
			$('table#'+tID+' > tbody > tr.'+nn).append( $('<td class="'+nc+'">').append(lap) );
		},//END ();
		setInfoTH : function ( tID, cName, rDis ) {
			$('table#'+tID+' > thead').append( $('<th class="'+cName+'">').append(rDis) );
		},//END ();
		setInfoTR : function ( tID, cName ) {
			$('table#'+tID+' > tbody').append( $('<tr class="'+cName+'">') );
		},//END ();
*/
